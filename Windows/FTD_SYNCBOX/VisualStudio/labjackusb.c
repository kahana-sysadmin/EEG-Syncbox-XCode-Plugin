...
#include "C:\myLibs\libusb-1.0.21\include\libusb-1.0\libusb.h"
//#include "libusb.h"
...
static void LJUSB_U3_FirmwareHardwareVersion(LJ_HANDLE hDevice, struct LJUSB_FirmwareHardwareVersion * fhv)
{
    ...
    BYTE command[(unsigned long)26];
    BYTE response[(unsigned long)38];
...
static void LJUSB_U6_FirmwareHardwareVersion(LJ_HANDLE hDevice, struct LJUSB_FirmwareHardwareVersion * fhv)
{
    unsigned long i = 0, r = 0;
...
static void LJUSB_UE9_FirmwareHardwareVersion(LJ_HANDLE hDevice, struct LJUSB_FirmwareHardwareVersion * fhv)
{
    unsigned long i = 0, r = 0;
    unsigned long epOut = UE9_PIPE_EP1_OUT, epIn = UE9_PIPE_EP1_IN;
    const unsigned long COMMAND_LENGTH = 38;
    const unsigned long RESPONSE_LENGTH = 38;
    BYTE command[(unsigned long)38];
    BYTE response[(unsigned long)38];
...
static bool LJUSB_isNullHandle(LJ_HANDLE hDevice)
{
    if (hDevice == NULL) {
...
static bool LJUSB_isMinFirmware(LJ_HANDLE hDevice, unsigned long ProductID)
{
...
LJ_HANDLE LJUSB_OpenDevice(UINT DevNum, unsigned int dwReserved, unsigned long ProductID)
{
    (void)dwReserved;
...
int LJUSB_OpenAllDevices(LJ_HANDLE* devHandles, UINT* productIds, UINT maxDevices)
{
    libusb_device **devs = NULL, *dev = NULL;
    struct libusb_device_handle *devh = NULL;
    struct libusb_device_descriptor desc;
...
bool LJUSB_ResetConnection(LJ_HANDLE hDevice)
{
    int r;
...
static unsigned long LJUSB_DoTransfer(LJ_HANDLE hDevice, unsigned char endpoint, BYTE *pBuff, unsigned long count, unsigned int timeout, bool isBulk)
{
    int r = 0;
    int transferred = 0;
...
static unsigned long LJUSB_SetupTransfer(LJ_HANDLE hDevice, BYTE *pBuff, unsigned long count, unsigned int timeout, enum LJUSB_TRANSFER_OPERATION operation)
{
...
unsigned long LJUSB_BulkRead(LJ_HANDLE hDevice, unsigned char endpoint, BYTE *pBuff, unsigned long count)
{
...
// Deprecated: Kept for backwards compatibility
unsigned long LJUSB_BulkWrite(LJ_HANDLE hDevice, unsigned char endpoint, BYTE *pBuff, unsigned long count)
{
    return LJUSB_DoTransfer(hDevice, endpoint, pBuff, count, LJ_LIBUSB_TIMEOUT_DEFAULT, true);
}


unsigned long LJUSB_Write(LJ_HANDLE hDevice, BYTE *pBuff, unsigned long count)
{
#if LJ_DEBUG
...
unsigned long LJUSB_Read(LJ_HANDLE hDevice, BYTE *pBuff, unsigned long count)
{
#if LJ_DEBUG
...
unsigned long LJUSB_Stream(LJ_HANDLE hDevice, BYTE *pBuff, unsigned long count)
{
#if LJ_DEBUG
...
unsigned long LJUSB_WriteTO(LJ_HANDLE hDevice, BYTE *pBuff, unsigned long count, unsigned int timeout)
{
#if LJ_DEBUG
...
unsigned long LJUSB_ReadTO(LJ_HANDLE hDevice, BYTE *pBuff, unsigned long count, unsigned int timeout)
{
#if LJ_DEBUG
...
unsigned long LJUSB_StreamTO(LJ_HANDLE hDevice, BYTE *pBuff, unsigned long count, unsigned int timeout)
{
#if LJ_DEBUG
...
void LJUSB_CloseDevice(LJ_HANDLE hDevice)
{
#if LJ_DEBUG
...
bool LJUSB_IsHandleValid(LJ_HANDLE hDevice)
{
    uint8_t config = 0;
    int r = 1;
...
unsigned short LJUSB_GetDeviceDescriptorReleaseNumber(LJ_HANDLE hDevice)
{
    libusb_device *dev = NULL;
    struct libusb_device_descriptor desc;
    int r = 0;
...
unsigned long LJUSB_GetHIDReportDescriptor(LJ_HANDLE hDevice, BYTE *pBuff, unsigned long count)
{
    libusb_device *dev = NULL;
    struct libusb_device_descriptor desc;
    int r = 0;
...
//not supported
bool LJUSB_AbortPipe(LJ_HANDLE hDevice, unsigned long Pipe)
{
    (void)hDevice;
    (void)Pipe;
