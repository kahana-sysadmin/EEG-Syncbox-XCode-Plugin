...
LJ_HANDLE openUSB(int ID){
	LJ_HANDLE hDevice = NULL;
    hDevice = openUSBConnection(ID);
    
    return hDevice;
}

void closeUSB(LJ_HANDLE hDevice){
    closeUSBConnection(hDevice);
}

void toggleSyncBitOn(LJ_HANDLE hDevice){
    long error;
...
void toggleHandleLEDOn(LJ_HANDLE hDevice, int channel){
    long error;
...
void toggleHandleLEDOff(LJ_HANDLE hDevice, int channel){
    long error;
